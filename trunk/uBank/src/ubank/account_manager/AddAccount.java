...
public class AddAccount extends GeneralActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		txt = (TextView) findViewById(R.id.above_txt).findViewById(
				R.id.Text_View_16_Gray);
		txt.setText("用户号" + Login.userId);
		txt = (TextView) findViewById(R.id.txt_acc_type).findViewById(
				R.id.Text_View_18);
		txt.setText("请选择账户类型:");
		...
		txt = (TextView) findViewById(R.id.txt2)
				.findViewById(R.id.Text_View_18);
		txt.setText("请设置账户别名:");
		txt = (TextView) findViewById(R.id.psd_txt).findViewById(
				R.id.Text_View_18);
		txt.setText("请输入账户密码:");

		accType = (Spinner) findViewById(R.id.spinner_acc_type).findViewById(
				R.id.Spinner);
		account = (EditText) findViewById(R.id.edit1).findViewById(R.id.et_acc);
		nickName = (EditText) findViewById(R.id.edit2).findViewById(
				R.id.et_user);
		accPwd = (EditText) findViewById(R.id.psd_edit).findViewById(
				R.id.et_psd);
		setAccType();

		next_btn = (Button) findViewById(R.id.next_btn).findViewById(
				R.id.button);
		...
		next_btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
					...
					if (EHelper.hasInternet(AddAccount.this)) {
						try {
							...
							json = ConnectWs.connect(AddAccount.this,
									EAccType.NULL, EOperation.ADD_ACC,
									Login.userId, account.getText().toString(),
									accType.getSelectedItem().toString(),
									nickName.getText().toString(), accPwd
											.getText().toString());
							boolean result = EHelper.toBoolean(json);
							MyDialogOne d1 = new MyDialogOne(AddAccount.this,
									R.style.dialog);
							if (result) {
				...
				} else if (account.getText().toString() == null) {
					Toast.makeText(AddAccount.this, "对不起，您未输入账号！",
							Toast.LENGTH_SHORT).show();
				} else if (accPwd.getText().toString() == null) {
					Toast.makeText(AddAccount.this, "对不起，您未输入密码！",
							Toast.LENGTH_SHORT).show();
				} else {
					Toast.makeText(AddAccount.this, "对不起，您未输入别名！",
							Toast.LENGTH_SHORT).show();
				}
			}
		});
